...
uchar4 RS_KERNEL subtract(uchar4 src_char, uint32_t x, uint32_t y)
{
    ...
    float4 bothPart = (float4) { dst_float.r - src_float.r,
    							 dst_float.g - src_float.g,
    							 dst_float.b - src_float.b, 1 } * bothFactor;
    bothPart = max(bothPart, (float4) { 0, 0, 0, 0 });
    
    float4 out_float = min(dstPart + bothPart, (float4) { 1, 1, 1, 1 });
